...
# Voeg je NVD API key toe als environment variable (export NVD_API_KEY='key')
...
def get_cves(service: str, version: str) -> List[Vulnerability]:
    ...
    try:
        ...
        console.print(f"[bold red][!] HTTP error fetching CVEs for {keywords}: {http_err}[/]")
...
def generate_table(port_info_list: List[PortInfo]) -> Table:
    table = Table(title="\U0001F50D Nmap Scan Report (with CVEs)", style="bold white")
    ...
    for port_info in port_info_list:
        ...
        if port_info.vulnerabilities:
            vuln_summary = "\n".join([
                f"{v.cve_id} (CVSS: {v.cvss_score})" if v.cvss_score else v.cve_id
                for v in port_info.vulnerabilities
            ])
...
def save_output(port_info_list: List[PortInfo], output_file: str, format: str):
    if format == "table":
        ...
        with open(output_file, "w") as f:
            for segment in table.__rich_console__(console, console.options):
